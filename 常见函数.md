...
```mysql
SELECT LENGTH('join'); #输出结果为4
```
<br>
	<strong>注意汉字的字节数可能不同</strong>
	</li>
...
```mysql
SELECT SUBSTR('芜湖起飞咯',2,4); #输出为：湖起飞咯
```
<br>
			</li>
		</ol>
...
```mysql
SELECT TRIM('   非   打 大萨达翁   '); 输出为：非   打 大萨达翁 
```
<br>
			</li>
...
```mysql
SELECT TRIM('ab' FROM 'abaabab  非  aa 打 aa aa大萨达翁  baabaaaab'); 输出为：aabab  非  aa 打 aa aa大萨达翁  baabaaa
```
<br>
			如:

```mysql
SELECT TRIM('ab' FROM ' abaabab  非  aa 打 aa aa大萨达翁  baabaaaab '); 
```
<br>
			则不会删去任何一个字符，因为第一个跟最后一个字符都是空格
			</li>
		</ol>	
	</li>
...
```mysql
SELECT LPAD('起飞咯',1,'*'); #结果为：起
```
<br>
	如：

```mysql
SELECT LPAD('起飞咯',5,'*');#结果为：**起飞咯
```
<br>
	</li>
	<li>用指定字符串右填充：select rpad(字符串,长度,填充字符串);
	</li>
	<li>替换（替换对象重复时所有替换对象都会被替换）：select replace(字符串,被替换对象,替换字符);<br/>
	如：

```mysql
SELECT REPLACE('123love321',1,5); #输出为：523love325
```
<br>
	</li>
</ol>
...
SELECT ROUND(-5.5); #输出-6
SELECT ROUND(5.2); #输出5
```
<br>
	</li>
	<li>向上取整（返回大于等于参数的最小整数）:select seil(参数)
```mysql
SELECT CEIL(-1.5); #输出-1
```
<br>
	</li>
	<li>向下取整（返回小于等于该参数的最大整数）：select floor(参数)
...
<br>
	...
	<li>截断（小数点后保留几位）：select truncate(参数1,参数2);
...
```mysql
select now();    #输出为2021-7-16 21:28:16
```
<br>
	</li>
	<li>返回当前系统日期（不包括时间）：select curdate();
...
<br>
	...
	<li>返回当前时间（不包括日期）：select curtime();

```mysql
SELECT CURTIME(); #输出为21:30:21
```
<br>
	</li>
	...
	<li>将日期格式的字符以某种特定的方式解析：select str_to_date(参数,指定格式);<br/>
		<img src="https://github.com/SaltyFishy/sql-language/blob/week5/%E8%A1%A8.jpg" alt="表" title="表">

```mysql
SELECT STR_TO_DATE('1968-5-7','%Y-%c-%d'); #输出为1968-05-07
```
<br>
	</li>
...
```mysql
SELECT DATE_FORMAT('2018/6/6','%Y年%c月%d日'); #输出为：2018年6月06日
```
<br>
	</li>
</ol>
...
<li>查看用户：select user();</li>
</ol>
<h4>流程控制函数（补充）</h4>
<ol>
<li>if 实现if else的效果<br/>
	if(判断表达式,为真的结果，为假的结果);
